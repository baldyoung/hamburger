...
int WinMainA (HINSTANCE hInstance, HINSTANCE hPrevInstance,
...
{  
  //  UINT  nFileCount = DragQueryFile(hDropInfo, (UINT)-1, NULL, 0);  //查询一共拖拽了几个文件
	...
	MessageBox(NULL, TEXT(szFileName), szAppName, MB_ICONINFORMATION);
    //read_file(hwnd,szFileName);
    //完成拖入文件操作，系统释放缓冲区?
    startSyncTestModule(); // ---------------------- 启动socket （必须使用异步线程进行加载，不然会卡死） 
